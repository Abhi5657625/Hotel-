"""
EaseMyTrip TH Portal – Hotel Booking Automation (Credit Card, With Login)
URL  : https://www.easemytrip.co.th/
...
# ── Config ─────────────────────────────────────────────────────────────────────
PORTAL_URL     = "https://www.easemytrip.co.th/"
HOTELS_URL     = "https://www.easemytrip.co.th/hotels/"
...
def step_open_hotels(page):
    print("\n🌐 Launching https://www.easemytrip.co.th/ ...")
...
def step_login(page):
    ...
    import time
    time.sleep(0.5)   # give Angular/JS time to render password step
...
def step_goto_hotels_after_login(page):
    ...
    page.goto(HOTELS_URL, wait_until="domcontentloaded")
    try:
        page.locator('.htl_location').first.wait_for(state="visible", timeout=12000)
        print("✅ Hotels search form ready")
    except Exception:
        try:
            page.locator('#txtCity').wait_for(state="attached", timeout=6000)
        except Exception:
...
def step_search_city(page):
    ...
    try:
        ...
        city_input.fill("")
        city_input.type("delhi", delay=50)
...
# ── Step 10: Click a random View Rooms button (new tab or same tab) ───────────
def step_view_rooms(page):
    ...
    # Wide set of selectors — TH portal may use different text or structure
    SELECTORS = [
        '.listing-bx button:has-text("View Rooms")',
        '.listing-bx a:has-text("View Rooms")',
        '.listing-bx button:has-text("Select Room")',
        '.listing-bx a:has-text("Select Room")',
        '.listing-bx button:has-text("View Details")',
        '.listing-bx a:has-text("View Details")',
        '.listing-bx button:has-text("Book Now")',
        '.listing-bx a:has-text("Book Now")',
    ...
    # Role-based fallback
    if not candidates:
        for name in ("View Rooms", "Select Room", "View Details", "Book Now"):
            try:
                for el in page.get_by_role("button", name=name).all():
                    try:
                        if el.is_visible():
                            candidates.append(el)
                    except Exception:
    ...
    # JS fallback — find any listing action button
    if not candidates:
        try:
            result = page.evaluate("""() => {
                var keywords = /view rooms|select room|view details|book now/i;
                var els = Array.from(document.querySelectorAll('.listing-bx a, .listing-bx button'));
                var el = els.find(e => keywords.test(e.innerText) && e.offsetParent !== null);
                if (el) { el.scrollIntoView({block:'center'}); return el.innerText.trim(); }
                return null;
            }""")
            print(f"🔍 JS fallback found button text: {result}")
        except Exception:
            pass

    # De-duplicate by pixel position
    seen_positions = set()
    unique_candidates = []
    ...
    print(f"🎲 Randomly selected button #{chosen_idx} of {len(unique_candidates)} available")
    ...
    # Try new-tab click first; fall back to same-tab navigation
    try:
        with page.context.expect_page(timeout=10000) as np_info:
            btn.click()
    ...
    try:
        ...
        btn.click()
        page.wait_for_url(lambda u: u != current_url, timeout=12000)
        page.wait_for_load_state("domcontentloaded")
        print(f"✅ 'View Rooms' clicked → same tab: {page.url}")
        return page
    except Exception as e:
...
def step_otp_page(page):
    ...
    # Wait for Stripe 3DS page to fully appear
    try:
        page.wait_for_url(
            lambda url: "stripe.com" in url.lower() or "hooks.stripe" in url.lower(),
            timeout=25000
        )
    except Exception:
        pass
    ...
    # The M2P Fintech 3DS iframe loads inside Stripe asynchronously.
    # Retry frame search every 2s for up to 30s total — never try main page
    # (cancel button lives only inside the nested iframe, never on main page).
    print("🚫 Clicking 'Cancel Payment'...")
    cancel_xpath = 'xpath=/html/body/div[1]/div[1]/footer/div[2]/p'
    cancel_frame = None
    deadline = time.time() + 30
    while time.time() < deadline and cancel_frame is None:
        for frame in page.frames:
            if not frame.url or frame.url == page.url:
                continue
            try:
                btn = frame.locator(cancel_xpath).first
                btn.wait_for(state="visible", timeout=2000)
                btn.click()
                print(f"✅ Cancel clicked in frame: {frame.url}")
                cancel_frame = frame
                break
            except Exception:
                continue
    ...
    if cancel_frame is None:
        print(f"⚠️ Cancel button not found after 30s. URL: {page.url}")

    # Confirm cancellation with Yes button
    print("✅ Clicking 'Yes' to confirm cancellation...")
    try:
        yes_selectors = ['button:has-text("Yes")', ':text("Yes")', 'a:has-text("Yes")', '[ng-click*="yes"]']
        clicked = False
        # Try in the same frame where cancel was clicked first
        if cancel_frame is not None:
            for sel in yes_selectors:
                try:
                    btn = cancel_frame.locator(sel).first
                    btn.wait_for(state="visible", timeout=5000)
                    btn.click()
                    print(f"✅ Yes clicked in cancel frame via: {sel}")
                    clicked = True
                    break
                except Exception:
                    continue
        # Fallback: try all other frames, then main page
        if not clicked:
            for frame in page.frames:
                ...
                for sel in yes_selectors:
                    try:
                        ...
                        btn.wait_for(state="visible", timeout=2000)
                        btn.click()
                        print(f"✅ Yes clicked in frame {frame.url} via: {sel}")
                        clicked = True
                        break
                    except Exception:
                        continue
                if clicked:
                    break
        if not clicked:
            clicked = click_first_visible(page, yes_selectors, "Yes button")
        if clicked:
            # Wait for navigation back to EMT booking confirmation page
            try:
                page.wait_for_url(
                    lambda url: "easemytrip" in url.lower(),
                    timeout=15000
                )
            except Exception:
                page.wait_for_timeout(3000)
...
def automate_easemytrip_ae():
    print("=" * 65)
    print("  EaseMyTrip TH – Hotel Booking (CC, With Login)")
    print("=" * 65)
