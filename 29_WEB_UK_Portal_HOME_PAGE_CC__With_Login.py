...
URL  : https://www.easemytrip.co.uk/
...
# ── Config ─────────────────────────────────────────────────────────────────────
PORTAL_URL     = "https://www.easemytrip.co.uk/"
HOTELS_URL     = "https://www.easemytrip.co.uk/hotels/"
...
def step_open_hotels(page):
    print("\n🌐 Launching https://www.easemytrip.co.uk/ ...")
...
def step_otp_page(page):
    ...
    if not cancel_clicked:
        # Stripe 3DS loads content inside an iframe — search all frames
        try:
            for frame in page.frames:
                try:
                    cancel_btn = frame.locator('xpath=/html/body/div[1]/div[1]/footer/div[2]/p').first
                    cancel_btn.wait_for(state="visible", timeout=5000)
                    cancel_btn.click()
                    print(f"✅ Cancel Payment clicked (inside frame: {frame.url})")
                    cancel_clicked = True
                    page.wait_for_timeout(1500)
                    break
                except Exception:
            ...
            print(f"⚠️ Cancel Payment click failed in all frames: {e}")

    if not cancel_clicked:
        print("⚠️ Cancel Payment button not found on page or in any frame")
    ...
    if not yes_clicked:
        # Yes button may also be inside the iframe
        try:
            for frame in page.frames:
                try:
                    for sel in ['button:has-text("Yes")', ':text("Yes")', 'a:has-text("Yes")']:
                        yes_btn = frame.locator(sel).first
                        if yes_btn.count() > 0 and yes_btn.is_visible():
                            yes_btn.click()
                            print(f"✅ Yes button clicked (inside frame: {frame.url})")
                            yes_clicked = True
                            page.wait_for_timeout(3000)
                            break
                    if yes_clicked:
            ...
            print(f"⚠️ Yes button click failed in all frames: {e}")

    if not yes_clicked:
        print("⚠️ Yes button not found on page or in any frame")


# ── Step 19: Capture Booking ID ───────────────────────────────────────────────
def step_capture_booking_id(page):
    print("\n🎫 Capturing Booking ID...")
    # Wait for redirect back to EaseMyTrip after cancellation
    try:
        page.wait_for_url(
