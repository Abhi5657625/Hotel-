...
def click_view_rooms(page, city_name):
    ...
    # Try CSS / text selectors (extended list including logged-in variants)
...
def click_continue_booking(page, city_name):
    ...
    def _after_click(label):
        ...
        try:
            ...
            print(f"✅ Navigated to checkout. URL: {page.url}")
        except Exception:
            print(f"⚠️ URL did not change to checkout. Current URL: {page.url}")
        print(f"📄 Current URL: {page.url}")
...
def click_credit_debit_card(page, city_name):
    ...
    # Wait for the payment section to render (may take time after navigation)
    try:
        page.wait_for_selector('.card.PG, .upi.PG, .netbanking.PG', timeout=20000)
    except Exception:
        pass
    page.wait_for_timeout(1000)
