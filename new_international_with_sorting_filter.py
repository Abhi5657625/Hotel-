...
def automate_easemytrip():
    with sync_playwright() as p:
        ...
        try:
            ...
            # Type Dubai in the search field
            print("📝 Typing Dubai in search field...")
            search_field.type("Dubai")
            print("✅ Dubai typed!")
            
            # Wait for autocomplete dropdown to load
            page.wait_for_timeout(1500)
            
            # Select Dubai location from visible dropdown (by text)
            print("📍 Selecting Dubai location...")
            location = page.locator('text=Dubai')
            location.first.click(force=True)
            print("✅ Dubai location selected!")
            ...
            # Scroll down to avoid header interception
            page.evaluate("window.scrollBy(0, 300)")
            page.wait_for_timeout(1000)
            
            # Click on sorting filter dropdown
            print("🔽 Clicking on sorting filter dropdown...")
            sorting_dropdown = page.locator('xpath=/html/body/app-root/div/hotel-srp/div[3]/div[2]/div/div[2]')
            sorting_dropdown.click(force=True)
            print("✅ Sorting dropdown opened!")
            
            # Wait for dropdown options to appear
            page.wait_for_timeout(1500)
            
            # Select "Low to High" option using the correct xpath
            print("💰 Selecting 'Low to High' sorting option...")
            low_to_high = page.locator('xpath=/html/body/app-root/div/hotel-srp/div[3]/div[2]/div/div[2]/div[2]/ul/li[2]/label/div')
            low_to_high.click()
            print("✅ 'Low to High' sorting applied!")
            ...
            # Click continue button with dynamic XPath
            print("🔘 Clicking continue button...")
            continue_button = new_page.locator('//form//button[contains(translate(text(), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "continue")]')
            ...
            # Click on PAN field
            print("📋 Clicking on PAN field...")
            pan_field = new_page.locator('//*[@id="guestDetails"]/div[3]/div[5]/input')
            pan_field.fill("AWERT1234E")
            print("✅ PAN 'AWERT1234E' entered!")
            
            # Wait for continue booking button to appear
            new_page.wait_for_timeout(1000)
            
            # Click continue booking button
            print("🔘 Clicking continue booking button...")
            continue_booking = new_page.locator('//*[@id="divHotelDetails"]/form/div/div[7]/button')
            continue_booking.click()
            print("✅ Continue booking button clicked!")
            
            # Wait for card payment options to appear
            new_page.wait_for_timeout(2000)
            ...
            # Cancel payment with dynamic XPath
            print("❌ Cancelling payment on OTP page...")
            try:
                cancel_button = new_page.locator('//div[@id="otpPin"]//button[contains(translate(text(), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "cancel") or contains(translate(text(), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "back")] | //div[@id="otpPin"]//button[2] | //button[contains(translate(@class, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "button")]').last
                cancel_button.click(force=True)
                print("✅ Payment cancelled!")
            except Exception as e:
                print(f"⚠️ Cancel button not found: {e}")
